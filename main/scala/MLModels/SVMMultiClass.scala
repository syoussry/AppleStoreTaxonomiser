...
case class SVMMultiClass(categoriesIDs: List[Double], training:RDD[LabeledPoint], test:RDD[LabeledPoint], numIterations: Int) extends MLModel {
  ...
  val mModel: List[SVMModel] = mTraining.map( training => SVMWithSGD.train(training, numIterations) )
  ...
  val predictionAndLabel: RDD[(Double, Double)] = mScoreAndLabels.map{
    (listOfScoresAndLabels: List[(Double, Double)]) => {
      val onlyScores = listOfScoresAndLabels.map{ case (score,label) => score }
